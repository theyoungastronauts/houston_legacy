...
import '../extensions/string_case_extensions.dart';
...
extension on String {
  String transpiled() {
    final builtInLambdaNamesEscaped = _builtInLambdas.keys.map(RegExp.escape).join('|');
    final lambdaPattern = RegExp('{?{{ *([^{}]*)\\.($builtInLambdaNamesEscaped)\\(\\) *}}}?');
    ...
    while (lambdaPattern.hasMatch(currentIteration)) {
      currentIteration = currentIteration.replaceAllMapped(lambdaPattern, (match) {
